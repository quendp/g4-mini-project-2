...
import AgentTentativeModal from "./AgentStatusesModal/AgentTentativeModal";

const AgentSidebarTentativeNav = (props) => {
  const [filteredStatus, setFilteredStatus] = useState("Tentative");

  const [showTentativeModal, setShowTentativeModal] = useState(false);

  const filteredCards = props.travelDetails.filter((travelDetail) => {
    return travelDetail.status === filteredStatus;
  });

  const openTentativeModal = () => {
    setShowTentativeModal(true);
  };

  const closeTentativeModal = () => {
    setShowTentativeModal(false);
  };
  ...
  return (
    <section id="bookingsSection" style={{ height: "100vh" }}>
        ...
        <div className="status-handler overflow-auto">
          <div className="row pt-lg-2 mb-5 mx-2">
            {filteredCards.map((travelDetail, index) => (
              ...
              >
                <div
                  className="card agentStatuses-cards"
                  onClick={openTentativeModal}
                >
                  ...
                  <div className="card-footer text-secondary d-flex alight-items-center justify-content-between">
                    {travelDetail.status === "Tentative" ? (
                      <span className="cards-span2">Required Confirmation</span>
                    ) : (
                      ""
                    )}
      ...
      <AgentTentativeModal
        showTentativeModal={showTentativeModal}
        closeTentativeModal={closeTentativeModal}
      />
    </section>
  );
};
